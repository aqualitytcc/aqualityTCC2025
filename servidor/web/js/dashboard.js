// Ficheiro: js/dashboard.js (VERSÃO ATUALIZADA E MAIS LIMPA)
/**
...
async function inicializarDashboard() {
    ...
    setInterval(atualizarDadosDashboard, 61000); // E depois repete a cada 30 segundos
}
...
function atualizarPainelStatusRapido(dados) {
    ...
    if (Object.keys(leituras).length === 0) {
        painel.innerHTML = '<p class="text-muted">Nenhuma leitura recente encontrada.</p>';
        return;
    }

    for (const dispositivoId in leituras) {
        const leitura = leituras[dispositivoId];
        const device = allDevices.find(d => d.id == dispositivoId);

        let statusHtml = '';
        ['ph', 'temperatura', 'condutividade', 'turbidez'].forEach(param => {
            if (leitura[param] !== null) {
                // Verifica se há um alerta para este parâmetro
                const emAlerta = notificacoes.some(notif => notif.toLowerCase().includes(param));
                const statusIcon = emAlerta
                ...
                statusHtml += `<div class="d-flex justify-content-between align-items-center">
                                <span>${param.charAt(0).toUpperCase() + param.slice(1)}: <b>${leitura[param]}</b></span>
                                ${statusIcon}
                               </div>`;
            }
        });
        ...
        const deviceHtml = `
            <div class="mb-3">
                <h6>${device ? device.nome : 'Dispositivo ' + dispositivoId}</h6>
                ${statusHtml}
            </div>
            <hr>
        `;
        painel.innerHTML += deviceHtml;
    }
}
...
function renderAllDevices() {
    ...
    allDevices.forEach(device => {
        ...
        cardBody.appendChild(btnReadings);
        cardBody.appendChild(btnSettings);
...
async function atualizarDadosDashboard() {
    console.log("A atualizar dados do dashboard...", new Date().toLocaleTimeString());
...
async function criarGraficoPrincipal() {
    ...
    try {
        const apiUrl = `api/leituras/buscar.php?dispositivo_id=${principalDevice.id}&data_inicio=${dataInicio}`;
        const response = await fetch(apiUrl);
        const resultado = await response.json();
        ...
        if (resultado.status === 'sucesso' && resultado.dados.length > 0) {
            ...
            chartContainer.innerHTML = `<p class="text-center text-muted mt-5">Sem dados nas últimas 24h para o dispositivo principal.</p>`;
        }
    } catch (error) {
